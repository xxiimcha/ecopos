...
namespace EcoPOSv2
{
    public partial class ZReading : Form
    {
        ...
        private void LoadZReadingRecords()
        {
            ...
            string adjustment123 = SQL.ReturnResult(@"SELECT SUM(ti.selling_price_inclusive)*-1 FROM transaction_items as ti INNER JOIN void_transaction as vt
            ...
            //var adjustments = SQL.ReturnResult(@"SELECT IIF((SELECT COUNT(*) FROM void_transaction WHERE terminal_id=@terminal_id AND void_date_time BETWEEN @from AND @to) > 0, (SELECT SUM(td.grand_total) FROM void_transaction as vt INNER JOIN transaction_details as td
            //                                        ON vt.order_ref = td.order_ref WHERE td.terminal_id=@terminal_id AND vt.void_date_time BETWEEN @from AND @to), 0)");
            ...
            if (count_sdt > 0)
            {
                SQL.AddParam("@from", store_open_date_time);
                SQL.AddParam("@to", datetime_now);
                SQL.AddParam("@adjustments", adjustment1);
                SQL.AddParam("@terminal_id", Properties.Settings.Default.Terminal_id);
                ...
                SQL.Query(@"SELECT (SELECT SUM(ABS(subtotal))+@adjustments FROM transaction_details WHERE terminal_id=@terminal_id AND date_time BETWEEN @from AND @to) as 'subtotal', 
                           SUM(disc_amt + cus_pts_deducted) as 'discount', SUM(ABS(grand_total)) as 'netsales', SUM(vatable_sale) as 'vatsales', SUM(vat_12) as 'vatamount',
            ...
            if (check_adjustment > 0)
            {
                ...
                string adjustment13 = SQL.ReturnResult(@"SELECT SUM(ti.selling_price_inclusive)*-1 FROM transaction_items as ti INNER JOIN void_transaction as vt
                ...
                {
                    adjustment = decimal.Parse(adjustment13.ToString());
                }
            ...
            if (check_adjustment > 0)
            {
                ...
                //                                    ON vt.order_ref = td.order_ref WHERE vt.terminal_id=@terminal_id"));

                decimal adjustments1 = 0M;
                string adjustments13 = SQL.ReturnResult(@"SELECT SUM(ti.selling_price_inclusive)*-1 FROM transaction_items as ti INNER JOIN void_transaction as vt
                ...
                {
                    adjustments1 = decimal.Parse(adjustments13);
                }
            ...
            SQL.Query(@"SELECT payment_method, CONVERT(DECIMAL(18,2), SUM(payment_amt - change)) FROM transaction_details 
                       WHERE terminal_id=@terminal_id AND date_time BETWEEN @from AND @to AND action = 1 GROUP BY payment_method ORDER BY CASE WHEN payment_method = 'Cash' THEN 1 Else 2 END ASC");
            if (SQL.HasException(true))
            {
        ...
        private void ZReading_Load(object sender, EventArgs e)
        {
            ...
            if(lblStoreOpen.Text == "")
            {
                MessageBox.Show("Please ZRead(switch cashier) and login again to proceed to ZReading", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.Close();
            }
        }
