...
namespace EcoPOSv2
{
    public partial class RedeemCart : Form
    {
        ...
        private void dgvRedeemItems_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            int check_item_in_cart = Convert.ToInt32(SQL.ReturnResult("SELECT COUNT(*) FROM redeem_cart WHERE productID = @productID"));
            if (SQL.HasException(true))
                return;
            ...
            if (check_item_in_cart == 0)
            {
                ...
                SQL.Query(@"INSERT INTO redeem_cart (productID, description, categoryID, static_pts, quantity)
                       SELECT productID, description, categoryID, pts, 1 FROM redeem_items WHERE productID = @productID");
            ...
            else
            {
                ...
                SQL.Query("UPDATE redeem_cart SET quantity = quantity + 1 WHERE productID = @productID");
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            SQL.Query("DELETE FROM redeem_cart WHERE itemID = @itemID");
        ...
        private void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            int order_ref = Convert.ToInt32(SQL.ReturnResult("SELECT order_ref FROM order_no WHERE order_ref = (SELECT MAX(order_ref) FROM order_no)"));
            if (SQL.HasException(true))
                return;
            ...
            SQL.Query("UPDATE order_no SET action = 5, cus_type = 4, cus_name = @cus_name, cus_ID_no = @cus_ID_no WHERE order_ref = @order_ref");
            ...
            int order_no = Convert.ToInt32(SQL.ReturnResult("SELECT order_no FROM order_no WHERE order_ref = (SELECT MAX(order_ref) FROM order_no)"));
            if (SQL.HasException(true))
                return;
            ...
            SQL.Query(@"INSERT INTO redeem_transaction (order_ref, order_no, no_of_items, total_pts, cus_ID_no, cus_card_no, card_balance_before, card_balance_after, userID)
                       VALUES (@order_ref, @order_no, @no_of_items, @total_pts, @cus_ID_no, @cus_card_no, @card_balance_before, @card_balance_after, @userID)");
            ...
            SQL.AddParam("@order_ref", order_ref);
            SQL.Query(@"INSERT INTO redeem_transaction_items (order_ref, itemID, productID, description, categoryID, static_pts, quantity, total_pts)
                       SELECT @order_ref, itemID, productID, description, categoryID, static_pts, quantity, total_pts FROM redeem_cart");
            ...
            SQL.Query("SELECT productID, quantity FROM redeem_cart");
            if (SQL.HasException(true))
                return;
