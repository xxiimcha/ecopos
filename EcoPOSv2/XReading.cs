    ...
    public partial class XReading : Form
    {
        ...
        private void LoadXReadingRecords()
        {
            ...
            if (count_sdt > 0)
            {
                ...
                //var adjustments = SQL.ReturnResult(@"Select IIF((SELECT COUNT(*) FROM void_transaction WHERE void_date_time BETWEEN @from And @to AND terminal_id=@terminal_id) > 0, (SELECT SUM(td.grand_total) FROM void_transaction As vt INNER JOIN transaction_details As td
                //                                    On vt.order_ref = td.order_ref WHERE td.terminal_id=@terminal_id AND vt.void_date_time BETWEEN @from And @to), 0)");

                //decimal adjustment1 = decimal.Parse(SQL.ReturnResult(@"SELECT SUM(ti.selling_price_inclusive)*-1 FROM transaction_items as ti INNER JOIN void_transaction as vt
                ...
                string adjustments123 = SQL.ReturnResult(@"SELECT SUM(ti.selling_price_inclusive)*-1 FROM transaction_items as ti INNER JOIN void_transaction as vt
                ...
                {
                    adjustments1 = decimal.Parse(adjustments123.ToString());
                }
                ...
                SQL.AddParam("@from", shift_start_date_time);
                SQL.AddParam("@to", datetime_now);
                SQL.AddParam("@adjustment", adjustments1);
                SQL.AddParam("@terminal_id", Properties.Settings.Default.Terminal_id);
